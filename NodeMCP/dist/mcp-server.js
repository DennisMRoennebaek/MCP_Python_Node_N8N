...
function makeServer() {
    ...
    server.registerTool("add_numbers", {
        title: "Add Numbers",
        description: "Add two numbers using the backend API. Accepts numeric values or strings that contain numbers (e.g. '1000').",
...
async function main() {
    ...
    app.get("/bridge/ping", async (_req, res) => {
        try {
            const r = await fetch(`${FASTAPI_BASE}/ping`);
            const data = await r.json();
            res.json(data);
    ...
    app.post("/bridge/add", async (req, res) => {
        try {
            const parsed = z
